...
def getSamples(filename):
	...
	data = scaleSamples(data, scaled_factor)
...
def scaleSamples(samples, scaled_max_value):
	...
	ksamp=pow(2,31)/scaled_max_value
	print "original max y min samples:",int(max(samples)),int(min(samples))

	# We scale all the samples
	for i in range(0, len(samples)):
		#ksamp=131070
		#samples[i] = int(samples[i]/ksamp)
		samples[i] = int(samples[i]/ksamp)+scaled_max_value
...
def calculateHops_2(hop0, hop1, hop_number, max_sample, min_sample):
	"""Returns the value of the new hop based on the calculations with
	the previous one, actual sample value and distance to the first hop.
	...
	while (s < n_samples): # We iterate over all the audio samples
		...
		if (delta >= 0):
			for j in range (4, 9):
				...
				hop_result = calculateHops_2(hop0, hop1, j, max_sample, min_sample)
				e2 = delta - hop_result
				if (e2 < 0):
					e2 = - e2
				if (e2 < emin):
					...
					break
			hop_result = calculateHops_2(hop0, hop1, hop_number, max_sample, min_sample)
			result[amp] = hop0 + hop_result # Final amplitude
