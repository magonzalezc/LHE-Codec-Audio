...
def writeFile(sym, first_amp, n_samples, max_sample, min_sample, filename):
	...
	f = open(filename, "wb") # This is the final lhe file
...
def writeFileStereo(sym_channel0, sym_channel1, first_amp_channel0, first_amp_channel1, n_samples, max_sample, min_sample, filename):
	"""Writes a .lhe file with some data for the decoder.
	...
	f.write(struct.pack("i", max_sample)) # Number of maximum sample of the audio (4 bytes)
	f.write(struct.pack("i", min_sample)) # Number of minimum sample of the audio (4 bytes)
	f.close() # Total header length: 29 bytes.
